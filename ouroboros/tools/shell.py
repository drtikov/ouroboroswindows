...
def _tracked_subprocess_run(cmd, **kwargs):
    """subprocess.run() replacement with process tracking.
    """
...
def _ensure_claude_cli(ctx: ToolContext) -> Optional[str]:
    ...
    if platform.system() == "Windows":
        return "⚠️ Claude Code CLI auto-install is currently disabled on Windows. Please install via `npm install -g @anthropic-ai/claude-code` manually."
...
def _build_augmented_path() -> str:
    ...
    for d in [
        ...
        if d not in current and (d == str(_NODE_BIN) or pathlib.Path(d).exists()):
            parts.append(d)
    if parts:
        return os.pathsep.join(parts) + os.pathsep + current
    return current
